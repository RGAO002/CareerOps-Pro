...
async def export_pdf(resume_id: str):
    """Render the resume to PDF (WeasyPrint) and stream it back as a download.

    GET so the frontend can use a plain <a href download> for one-click
    download — no JS, no preview window, no print dialog. The PDF visual
    is generated by WeasyPrint, so it will not pixel-match the editor canvas
    (different CSS engine), but layout structure + content match.
    """
    _ensure_valid_id(resume_id)
    r = resume_store.get(resume_id)
    if r is None:
